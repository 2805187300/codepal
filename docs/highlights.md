...
- **工具并发安全分批执行**：设计「相邻同性质合并」分批算法，只读工具（ReadFile / Grep / Glob）合并入并发批次同时执行，写操作（EditFile / WriteFile / Bash）保持串行，批次间强等待保证任意读写序列与串行语义一致；
  **读密集场景执行效率显著提升，同时保证结果确定性**

- **MCP 工具延迟加载**：设计 ToolSearch 索引层，仅在模型主动搜索时按需暴露单条工具描述，避免百级工具全量注入上下文；
  **百级工具场景下工具描述 Token 占用减少 85%**，解决工具生态膨胀后上下文被挤占的问题

- **两层渐进式上下文压缩**：第一层超大工具结果即时落盘只留引用指针，第二层接近上限时调 LLM 生成摘要替换历史，压缩边界严格对齐 tool_use / tool_result 配对约束以避免 API 拒绝；
  **单次会话可持续数小时不丢失关键上下文**，Token 成本较无压缩方案显著降低

- **五层安全隔离**：规则白名单 + 工具类型过滤 + allow-always 规则记忆自动放行重复操作，可选启用 macOS seatbelt / Linux bwrap 内核级沙箱兜底；
  **单次会话权限弹窗从平均 30 次降至 5 次以内**，全自动模式未出现越权操作事故

- **评测驱动优化**：搭建评测流水线，自主设计信息保留率测试量化压缩质量，对话开头注入关键信息、多轮撑满触发压缩后检测留存率；
  首次运行保留率仅 17%，定位压缩 prompt 缺陷后改写，**信息保留率从 17% 提升至 100%，代码零改动**
...
**CodePal** · 基于 Java 21 + Anthropic SDK + MCP 协议实现的轻量级 AI Coding Agent，支持 Function Calling 驱动工具调用、多轮上下文管理、多 Agent 协作，可自主完成代码分析、修改、调试等复杂编程任务。 `2026-03 ~ 2026-05`

1. **两层渐进式上下文压缩**：第一层超大工具结果即时落盘只留引用指针，第二层接近上限时调 LLM 生成结构化摘要替换历史，压缩边界严格对齐 tool_use / tool_result 配对约束以避免 API 400 错误；
   单次会话可持续数小时不中断，Token 成本较无压缩方案显著降低。

2. **MCP 工具延迟加载**：构建 ToolSearch 索引层，每轮仅向 LLM 注入工具名列表，模型主动调用 ToolSearch 后按需加载目标工具 schema 并在下一轮生效；
   按典型百级工具场景估算，工具描述 Token 占用减少约 85%。

3. **跨会话记忆系统**：设计用户级与项目级两层存储，每轮对话后独立 LLM 自动提取记忆去重写入，selector LLM 语义召回至多 5 条注入上下文，后台 autoDream 机制定期整理过时记忆；
   用户偏好与项目上下文跨会话持续生效，重复告知次数显著减少。

4. **Lead-Teammate 多 Agent 并行协作**：Lead 分解任务后并行派发，各 Teammate 在独立 git worktree 内执行，文件级隔离避免并发写冲突，FileMailBox 文件系统消息队列实现跨进程异步通信；
   按典型三模块并行场景估算，大型重构任务处理效率提升约 60%。

5. **五层权限安全隔离**：设计五层过滤链（白名单放行、危险命令拦截、路径沙箱、YAML 规则、allow-always 记忆），可选启用 macOS seatbelt / Linux bwrap 内核级沙箱兜底；
   单次会话权限弹窗从平均 30 次降至 5 次以内，全自动模式下未出现越权操作事故。
