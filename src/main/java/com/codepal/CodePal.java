...
import com.codepal.config.ConfigLoader;
import com.codepal.config.ProviderConfig;
import com.codepal.llm.LlmClient;
import com.codepal.print.PrintMode;
import com.codepal.prompt.PromptBuilder;
import com.codepal.remote.RemoteServer;
import com.codepal.teams.TeamManager;
import com.codepal.teams.TeamTools;
import com.codepal.teams.TeammateRunner;
import com.codepal.tool.ToolRegistry;
import com.codepal.tool.impl.BashTool;
import com.codepal.tool.impl.EditFileTool;
import com.codepal.tool.impl.GlobTool;
import com.codepal.tool.impl.GrepTool;
import com.codepal.tool.impl.ReadFileTool;
import com.codepal.tool.impl.WriteFileTool;
import com.codepal.tui.CodePalModel;
...
public class CodePal {
    ...
    public static void main(String[] args) {
        ...
        // 格式：codepal --teammate --team-name <t> --agent-name <n>（见 SpawnDispatcher.buildTeammateCLI）
        ...
        // 环境变量回退
        if (configPath == null) {
            String envPath = System.getenv("CODEPAL_CONFIG");
     ...
     * 然后进入 TeammateRunner 的常驻循环。对应 Go 版 cmd/codepal/teammate.go 的 runTeammate。
     */
    private static void runTeammate(String[] args) {
        ...
        // 加载与 TUI 相同的配置（默认路径 / CODEPAL_CONFIG 环境变量）
        AppConfig config;
        try {
            config = ConfigLoader.load(System.getenv("CODEPAL_CONFIG"));
        } catch (ConfigLoader.ConfigException e) {
            System.err.println("Configuration error: " + e.getMessage());
            System.exit(1);
            return;
        ...
        // 本地 TeamManager 接入同一个团队：Team 的邮箱目录由团队名派生
        // （<user.dir>/.codepal/teams/<team>/inboxes），与 lead 进程共享，从而收发消息。
        // 用 createTeamWith 登记而非 createTeam，避免清空 lead 已写入的共享任务库。
        TeamManager teamMgr = new TeamManager();
        TeamManager.Team team = new TeamManager.Team(teamName, TeamManager.TeamMode.IN_PROCESS);
