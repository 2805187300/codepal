...
package com.codepal.command;

import com.codepal.command.Command.CommandType;
...
public class CommandRegistry {
    ...
    /** Creates a registry pre-populated with the default CodePal commands. */
    ...
    private void registerDefaults() {
        ...
        register(
                ...
                ctx -> {
                    ...
                    if (skills.isEmpty()) return "No skills installed.\n\nAdd skills to .codepal/skills/<skill-name>/SKILL.md";
        ...
        register(
                new Command("metrics", "Show agent runtime metrics (tool call stats, token usage, traces)",
                        new String[]{}, CommandType.LOCAL, false),
                ctx -> com.codepal.metrics.MetricsCommand.handle()
        );
        ...
        register(
                ...
                ctx -> {
                    String dir = (ctx.args() == null || ctx.args().isBlank())
                            ? ctx.workDir() + "/.codepal/eval"
                            : ctx.args().strip();
                    var cases = com.codepal.eval.EvalCase.Loader.loadFromDir(dir);
                    if (cases.isEmpty()) {
