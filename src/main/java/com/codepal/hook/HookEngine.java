...
public class HookEngine {
     ...
     * 注入 CODEPAL_EVENT、CODEPAL_TOOL、CODEPAL_FILE_PATH 环境变量。
     * 支持超时保护，超时后强制终止子进程。
     */
    private HookResult executeCommand(Hook h, HookContext ctx) {
        ...
        try {
            ...
            env.put("CODEPAL_EVENT", ctx.event() != null ? ctx.event().value() : "");
            env.put("CODEPAL_TOOL", ctx.toolName() != null ? ctx.toolName() : "");
            env.put("CODEPAL_FILE_PATH", ctx.filePath() != null ? ctx.filePath() : "");
