...
/**
 * 加载项目和用户级别的指令文件（CODEPAL.md / AGENTS.md），支持 @include 递归展开。
 *
 * <p>发现顺序（越靠后优先级越高，模型注意力优先关注后出现的内容）：
 * <ol>
 *   <li>用户全局：~/.codepal/CODEPAL.md、~/.codepal/AGENTS.md</li>
 *   <li>项目级：从 git root 沿目录树向下到 workDir，每个目录的 CODEPAL.md 和 AGENTS.md</li>
 *   <li>workDir/.codepal/INSTRUCTIONS.md（兼容旧版）</li>
 *   <li>workDir/CODEPAL.local.md（私有本地覆盖）</li>
 * </ol>
 *
 * <p>@include 指令：
 * <ul>
...
public final class InstructionLoader {
    ...
    static List<InstructionSource> discoverInstructions(String workDir) {
        ...
        if (home != null && !home.isEmpty()) {
            addSource(sources, seen, Path.of(home, ".codepal", "CODEPAL.md"));
            addSource(sources, seen, Path.of(home, ".codepal", "AGENTS.md"));
        }

        // 2. 项目级：从 git root 到 workDir 的每一级目录
        for (String dir : projectInstructionDirs(workDir)) {
            addSource(sources, seen, Path.of(dir, "CODEPAL.md"));
            addSource(sources, seen, Path.of(dir, "AGENTS.md"));
        }

        // 3. 兼容旧版 INSTRUCTIONS.md
        addSource(sources, seen, Path.of(workDir, ".codepal", "INSTRUCTIONS.md"));

        // 4. 本地私有覆盖
        addSource(sources, seen, Path.of(workDir, "CODEPAL.local.md"));
