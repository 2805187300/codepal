 ...
 * <p>存储结构：
 * <ul>
 *   <li>用户级 (~/.codepal/memory/)：存放 type=user / type=feedback 的记忆文件</li>
 *   <li>项目级 (.codepal/memory/)：存放 type=project / type=reference 的记忆文件</li>
 * </ul>
 *
...
public class MemoryManager {
    ...
    private static final String MEMORY_DIR = ".codepal/memory";
    ...
    /** 返回用户级记忆目录（~/.codepal/memory/） */
    public Path userMemDir() {
        return userMemDirPath;
    }

    /** 返回项目级记忆目录（.codepal/memory/） */
    ...
    /**
     * 加载指令文件：支持用户级（~/.codepal/CODEPAL.md）、项目级（git root 到 workDir 逐层）、
     * 兼容旧版 INSTRUCTIONS.md、私有 CODEPAL.local.md，以及 @include 递归展开。
