...
public final class SpawnDispatcher {
    ...
    public record SpawnConfig(
            ...
            String protocol,
            com.codepal.config.ProviderConfig providerConfig,
            String workdir
    ) {}
     ...
     * Format: cd '<workdir>' && '<codepal>' --teammate --team-name <t> --agent-name <n>
     */
    public static String buildTeammateCLI(String teamName, String memberName, String workdir) {
        ...
        // Find codepal executable (assume it's the current JAR or on PATH)
        String codepal = ProcessHandle.current().info().command().orElse("codepal");
        return "cd %s && %s --teammate --team-name %s --agent-name %s".formatted(
                shellQuote(wd), shellQuote(codepal), shellQuote(teamName), shellQuote(memberName));
    }
