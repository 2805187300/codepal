...
class MemoryConsolidatorTest {
    ...
    void maybeRun_skipsWhenTimeGateNotMet(@TempDir Path dir) throws IOException {
        Path memDir = dir.resolve(".codepal/memory");
    ...
    void maybeRun_skipsWhenSessionGateNotMet(@TempDir Path dir) throws IOException {
        Path memDir = dir.resolve(".codepal/memory");
        Path sessDir = dir.resolve(".codepal/sessions");
        Files.createDirectories(memDir);
    ...
    @Test
    @EnabledIfEnvironmentVariable(named = "CODEPAL_TEST_API_KEY", matches = ".+")
    void e2e_consolidationMergesDuplicates(@TempDir Path dir) throws Exception {
        String apiKey = System.getenv("CODEPAL_TEST_API_KEY");
        String baseUrl = System.getenv().getOrDefault("CODEPAL_TEST_BASE_URL", "https://api.minimaxi.com/v1");
        String model = System.getenv().getOrDefault("CODEPAL_TEST_MODEL", "MiniMax-M3");
